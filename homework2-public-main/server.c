...
void unsubscribe_client_from_topic(from_tcp_client m, client *c, int sock) {
    int i, nr_topics_to_right;
    topic *t = c -> topics;

    //Caut indexul topicului in vectorul de topicuri al clientului.
    for(i = 0; i < c -> nr_topics; ++i) {
        if(strcmp(m.topic, c -> topics[i].title) == 0) {
            /*
                Am gasit topicul in lista de topicuri.
                Mut la stanga elementele din vector de dupa topic,
                practic stergand topicul pt care fac unsubscribe.
                Dupa care scad nr-ul de topicuri.

                !!Atentie, nu am avut cum sa testez cazul asta, si nu
                e testat nici de teste, deci sper sa fie in regula.
            */
            nr_topics_to_right = c -> nr_topics - (i + 1);
            int size = sizeof(topic);
            memcpy(t + (i * size), t + ((i + 1) * size), nr_topics_to_right * size);
            c -> nr_topics --;
            return;
        }
    }
...
void free_the_memory(client *clients_array, int nr_clients) {
    int i;
    for(i = 0; i < nr_clients; ++i) {
        free(clients_array[i].topics);
        free(clients_array[i].stored_messages_array);
    }
    free(clients_array);
}
...
int main(int argc, char *argv[]) {
    ...
    */
            ...
            */
            ...
            for (i = 0; i < nr_clients; i++) {
				...
				if (FD_ISSET(c -> socket, &tmp_fds)) {
                    ...
                    if(ret == 0) { 
                        ...
                        FD_CLR(c -> socket, &read_fds);
                         ...
                         /*
						    ...
						    Daca s-a atins numarul maxim de topicuri
                            se redimensioneaza.
                        */
                        if(c -> nr_topics == c -> topics_array_phys_dim) {
                            ...
                            DIE(aux == NULL, "[server] Error at reallocating array of topics.\n");
                            *t = aux;
                        }
					...
					} else if(strncmp(m.c, "unsubscribe", 11) == 0) {
                        //S-a primit comanda de unsubscribe de la client.
                        unsubscribe_client_from_topic(m, c, c -> socket);
					}
    ...
    free_the_memory(clients, nr_clients);
