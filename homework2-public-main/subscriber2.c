...
double float_value(char *payload) {
    ...
    } else if(negative_power_of_10 == 0) {
...
void output_subscriber(to_tcp_client m) {
    /*
        Eventual sa se verifice cazuri posibile
        de corupere a datelor primite de la server
    */
    //--------------------------------------------
        ...
        */
        ...
        if(FD_ISSET(sock, &tmp_fds) != 0) {
			...
			*/
            ...
            if(check_if_error_from_server(&m) == 1) {
                /*
